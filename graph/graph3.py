...
def value_key(value1, value2):
    if value1 >= FACTOR or value2 >= FACTOR:
...
def graph_values1(paths, vectors):
    ...
    for path in paths:
        ...
        key1 = value_key(value[0], value[1])
        key2 = value_key(value[2], value[3])
...
def max_comp_value_diff1(values):
    max_comp_value, sel_value1, sel_value2 = None, None, None
    diff_values = []
    unique_keys = defaultdict(set)
    total = 0
    for i, value1 in enumerate(values):
        for value2 in values[i:]:
            ...
            key2 = value_key(value[2], value[3])
            if not key1 in unique_keys or not key2 in unique_keys[key1]:
                diff_values.append(TAU3 @ value)
                unique_keys[key1].add(key2)
    print('for', len(values), 'all diffs', total, 'reduced to', len(diff_values))
...
@profile
def test_graph_values1(paths, vectors):
    values = []
    diff_values = []
    unique_keys = defaultdict(set)
    for path in paths:
        ...
        if not key1 in unique_keys or not key2 in unique_keys[key1]:
            values.append(path_value(vectors, path))
            unique_keys[key1].add(key2)
    print(len(paths), 'values reduced to', len(values))

    opt_diff_values = []
    opt_unique_keys = defaultdict(set)
    for i, value1 in enumerate(values):
        for value2 in values[i:]:
            diff_values.append(value1 - value2)
    for value in diff_values:  
        key1 = value_key(value[0], value[1])
        key2 = value_key(value[2], value[3])
        if not key1 in opt_unique_keys or not key2 in opt_unique_keys[key1]:
            opt_diff_values.append(value)
            opt_unique_keys[key1].add(key2)
    print(len(diff_values), 'diff values reduced to', len(opt_diff_values))

    tau3_diff_values = set()
    for i, value1 in enumerate(values):
        for value2 in values[i:]:
            tau3_diff_values.add(TAU3 @ (value1 - value2))
    print(len(diff_values), 'tau3 values reduced to', len(tau3_diff_values))
    return values

# paths = find_all_paths(graph, 12)
# values = test_graph_values1(paths, vectors)
