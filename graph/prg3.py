...
def over1_x(x, seznam_poctu):
    print(x)
    opakujici_potomci = {}
    potomci = najdi_potomky(x)
    for p in potomci:
        opakujici_potomci[podej_hash(p)] = p
    ...
    while pocet_potomku > 0:
        ...
        for p in potomci:
            ...
            for pp in najdi_potomky(p):
                _hash = podej_hash(pp)
                if _hash in opakujici_potomci:
                    continue
                dalsi_potomci.append(pp)
                opakujici_potomci[_hash] = pp
...
def over1_xx(seznam_poctu):
    for x in X:
        if over1_x(x, seznam_poctu) > 0:
            return False
    return True
      
def over2_xx(seznam_poctu):
    for x in X:
        if over2_x(x, seznam_poctu) > 0:
            return False
    return True
...
def main():
    ...
    seznam_poctu1 = []
    tremoc1 = over1_xx(seznam_poctu1)
    seznam_poctu2 = []
    tremoc2 = over2_xx(seznam_poctu2)
    if tremoc1 and tremoc2:
        if not seznam_poctu1 == seznam_poctu2:
            raise ValueError('ruzne vysledky', seznam_poctu1, seznam_poctu2)
    if tremoc1:
        print('Gratulujeme1')
    if tremoc2:
        print('Gratulujeme2')
